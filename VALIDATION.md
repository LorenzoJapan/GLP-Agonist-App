...
## Round 6: no drug shown when nothing is indicated

On 2026-08-24, prompted by a direct request to handle the true-negative case explicitly, the "no valid answer exists" edge case documented under Round 4 (BMI-under-25, weight-loss-only goal, no diabetes — where FDA labeling doesn't support *any* catalog option) was changed from a UX judgment call into an explicit, tested state. Previously this combination fell into the existing closest-available-fallback path: `renderResults()` re-scored all six catalog options with no floor, took the top two by raw score, and displayed them as drug cards under the (danger-colored) "No option is a strong match" banner — even though `scoreOption()`'s own BMI/diabetes gates had already zeroed both options' scores and reasons. A provider skimming the card UI could see a branded drug name and a form factor before registering that the reasons list was empty.

**Fix:** added `isIndicated(opt, a)` — a pure function mirroring the same two eligibility gates already inside `scoreOption()` (the weight-management BMI/comorbidity gate, and the diabetes goal/diagnosis gate), factored out so it can be checked *before* any scoring happens. `renderResults()` now filters the catalog to `isIndicated` options first. If that filtered set is empty — meaning no catalog option is eligible for this patient under either category, not merely low-scoring — the results screen shows a dedicated "Drug therapy is not indicated" card instead of the closest-available fallback, and renders zero drug cards. `scoreOption()` itself (the validated ranking logic behind Rounds 1–5) was left untouched to avoid any risk of regressing its documented accuracy; `isIndicated()` duplicates its gate conditions rather than refactoring them out.

This only changes behavior for genuine non-indication (BMI too low for weight management, no diabetes goal/diagnosis for the diabetes category, per FDA labeling thresholds already encoded in `scoreOption()`). It does not change any of the previously-documented mismatch/tie edge cases (contradictory synthetic inputs, off-category comorbidity bonuses, grader limitations) — those involve at least one indicated, eligible option and still go through the normal scoring and closest-available-fallback paths unchanged.

**Verification:** the shipped `validation/` and `per_drug_test_50/` suites aren't bundled with this change, so this was checked with a small targeted Playwright script (`test_not_indicated.js`, not part of the shipped app) driving the real UI through six cases: three genuine non-indication combinations (BMI under 25 with no comorbidities; BMI under 25 with ASCVD present, confirming the BMI floor holds regardless of comorbidity; BMI 25–29.9 with no qualifying comorbidity) all correctly show the not-indicated card with zero drug cards, and three indicated combinations (BMI 35+ weight-loss goal; goal "both" + type 2 diabetes at BMI under 25; stated diabetes goal alone with no diagnosis) all correctly still show ranked drug cards. All six assertions passed with no regressions in the indicated cases' card counts or reasons.
