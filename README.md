...
The matching logic and indication text are grounded in five sources:
...
5. Nicholls SJ, Pavo I, Bhatt DL, Buse JB, Del Prato S, Kahn SE, et al; for the SURPASS-CVOT Investigators. Cardiovascular outcomes with tirzepatide versus dulaglutide in type 2 diabetes. N Engl J Med. 2025;393(24):2409–20. doi:10.1056/NEJMoa2505928

These same five references appear on the app's own results screen.

## Validation

The matching algorithm was independently checked against 100 randomly generated patient profiles, graded by logic written fresh from the source papers (not from the app's own code): 88% exact match, 91% evidence-consistent overall, after fixes to delivery-preference weighting, oral-delivery goal-matching, a BMI eligibility floor on weight-management recommendations, a goal-alignment tiebreaker prompted by a per-drug recall test (50 patients sampled per drug) that found Ozempic recall at only 64% (now 84%, with the other five drugs holding at 96–100%), and — most recently — Mounjaro's new FDA-approved cardiovascular-risk-reduction indication, re-validated across all three test suites at exact parity with the prior round. See [VALIDATION.md](VALIDATION.md) for the full method, all six rounds of results, and an honest breakdown of every remaining mismatch. The scripts that produced it are in `validation/`.
